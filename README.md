...
<h2>Installation</h2>
<p>You can install maxmods using pythons built-in package manager:</p>
...
<p>When creating a <code>AuthSesh</code> instance there are a few options for where the instance will connect to. You can only pick from one of these, if you provide a remote server and a path, it will connect to the server and disreguard the path given.</p>
...
<p>This will establish a secure connection to the remote server provided. If you want to host a server look at <a href="https://github.com/maxwellewxam/maxmods#maxmodsauthmain" target="_new">maxmods.auth.main</a>.</p>
...
<h3>Logging In</h3>
<p>Once the username and password are set, you can use the <code>login()</code> method to log in to the server:</p>
...
<p>If the login is successful, you can use the <code>save()</code> and <code>load()</code> methods to save and load data from the server. For example:</p>
